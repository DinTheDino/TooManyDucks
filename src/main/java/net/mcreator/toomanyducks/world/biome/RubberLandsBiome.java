...
package net.mcreator.toomanyducks.world.biome;
...
import net.mcreator.toomanyducks.init.ToomanyducksModBiomes;
import net.mcreator.toomanyducks.ToomanyducksMod;

import java.util.Map;
import java.util.HashMap;

public class RubberLandsBiome {
	...
	public static Biome createBiome() {
		BiomeSpecialEffects effects = new BiomeSpecialEffects.Builder().fogColor(12638463).waterColor(4159204).waterFogColor(329011).skyColor(7972607)
				.foliageColorOverride(-205).grassColorOverride(-205).build();
	...
	public static void init() {
		Registry.register(BuiltinRegistries.CONFIGURED_SURFACE_BUILDER, new ResourceLocation(ToomanyducksMod.MODID, "rubber_lands"), SURFACE_BUILDER);
		CONFIGURED_FEATURES.forEach((resourceLocation, configuredFeature) -> Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, resourceLocation,
				configuredFeature));
		BiomeManager.addBiome(BiomeManager.BiomeType.WARM, new BiomeManager.BiomeEntry(
				ResourceKey.create(Registry.BIOME_REGISTRY, BuiltinRegistries.BIOME.getKey(ToomanyducksModBiomes.RUBBER_LANDS)), 10));
	}

	private static final Map<ResourceLocation, ConfiguredFeature<?, ?>> CONFIGURED_FEATURES = new HashMap<>();

	private static ConfiguredFeature<?, ?> register(String name, ConfiguredFeature<?, ?> configuredFeature) {
		CONFIGURED_FEATURES.put(new ResourceLocation(ToomanyducksMod.MODID, name + "_rubber_lands"), configuredFeature);
